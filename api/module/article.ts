import { recommendArticles, TimeLineArticle, } from '../module/article_type';

const makeArticleJson = async (recommendArticles: recommendArticles) => {
    ...
    const hoge = process.env.IMG_URL;
        for (let articleElement of recommendArticles){
            const newArticleResult = new TimeLineArticle(
                articleElement.article_categories[0],
